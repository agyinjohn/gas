    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-sm shadow-sm' : 'bg-transparent'}`}>
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:block">
          <Link href="/user" className="inline-flex items-center gap-1.5 bg-brand-orange text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-orange-600 transition-colors">
            Order Now →
          </Link>
        </div>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 pb-6 pt-2">
          ...
          <Link href="/user" className="mt-4 block text-center bg-brand-orange text-white text-sm font-semibold px-5 py-3 rounded-full hover:bg-orange-600 transition-colors">
            Order Now →
          </Link>
        </div>
      )}
