...
const CYLINDER_SIZES = [3, 6, 12];
const DELIVERY_FEE = 15;

interface StationT { id: string; name: string; address: string; distanceKm: number; ratingAvg: number; cylinderListings: any[]; lat: number; lng: number; }
...
function StationSelector({ selected, onSelect }: { selected: StationT | null; onSelect: (s: StationT) => void }) {
  ...
  const stations: StationT[] = data?.stations ?? [];
      ...
      {stations.map((s) => {
        ...
        return (
          <button key={s.id} onClick={() => onSelect(s)}
            ...
            <div className={cn('w-9 h-9 rounded-xl flex items-center justify-center shrink-0', isSel ? 'bg-brand-500' : 'bg-[var(--bg-card2)]')}>
              <Flame className={cn('w-4 h-4', isSel ? 'text-white' : 'text-[var(--text-muted)]')} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-[var(--text-primary)] truncate">{s.name}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="flex items-center gap-1 text-xs text-[var(--text-muted)]"><MapPin className="w-3 h-3" />{s.distanceKm} km</span>
                <span className="flex items-center gap-1 text-xs text-amber-500"><Star className="w-3 h-3 fill-current" />{s.ratingAvg?.toFixed(1)}</span>
              </div>
            </div>
...
export default function CheckoutPage() {
  ...
  const stationNameParam = params.get('stationName') ?? '';
  const stationAddrParam = params.get('stationAddress') ?? '';

  // Step 1 — service type
  const [serviceType, setServiceType] = useState<'refill' | 'exchange'>('refill');
  // Station (quick order)
  const [selectedStation, setSelectedStation] = useState<StationT | null>(null);
  // Two location fields:
  // pickupLoc = where rider picks up the cylinder FROM the customer
  // deliveryLoc = where the cylinder gets delivered TO (can be same as pickup)
  ...
  const effectiveStationName = stationNameParam || selectedStation?.name || '';
  const effectiveStationAddr = stationAddrParam || selectedStation?.address || '';

  // Fetch station data FIRST — must be before cart/setQty which reference stationData
  ...
  const availableSizes: number[] = stationData?.cylinderListings
    ...
    ?.map((l: any) => l.size) ?? CYLINDER_SIZES;

  // Step 2 — cylinder (multi-select)
  const [cart, setCart] = useState<Record<number, number>>(() => {
    const initSize = parseInt(params.get('size') || '0');
    return initSize ? { [initSize]: 1 } : {};
  });
  ...
  function handleContinue() {
    ...
    // Store photo in sessionStorage to avoid 431 URL too large error
    if (photo) {
      sessionStorage.setItem('checkout_photo', photo);
    } else {
    ...
    const q = new URLSearchParams({
      ...
      stationAddress:   effectiveStationAddr,
      serviceType,
    ...
    <div className="min-h-full bg-[var(--bg)] pb-40">
      ...
      {showDeliveryPicker && (
        <LocationPicker
          ...
          pickupLocation={pickupLoc?.formatted}
        />
      )}
      ...
      <div className="px-4 py-5 space-y-7 max-w-lg mx-auto">
        ...
        {/* Station selector for quick order */}
        {isQuickOrder && (
          <div>
            <p className="text-sm font-bold text-[var(--text-primary)] mb-3">Select Station</p>
        ...
        {/* Step 1 — Service Type */}
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-3">1. Choose Service Type</p>
          <div className="space-y-3">
            {[
              { value: 'refill',   icon: Flame,          label: 'LPG Gas Refill',   desc: "We'll fill your empty cylinder",        disabled: false },
              { value: 'exchange', icon: ArrowRightLeft, label: 'Cylinder Exchange', desc: 'Coming soon',                           disabled: true  },
            ].map(({ value, icon: Icon, label, desc, disabled }) => {
              const active = serviceType === value && !disabled;
              return (
                <button key={value} onClick={() => !disabled && setServiceType(value as any)}
                  disabled={disabled}
                  className={cn('w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all relative',
                    disabled ? 'border-[var(--border)] opacity-50 cursor-not-allowed' :
                    active   ? 'border-brand-500' : 'border-[var(--border)]'
                  )}>
                  <div className={cn('w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0',
                    active ? 'border-brand-500' : 'border-[var(--text-muted)]')}>
                  ...
                  {disabled && (
                    <span className="text-[10px] font-bold text-[var(--text-muted)] bg-[var(--bg-card2)] px-2 py-0.5 rounded-full shrink-0">Soon</span>
                  )}
                </button>
              );
            })}
        ...
        {/* Step 2 — Cylinder Details */}
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-1">2. Cylinder Details</p>
          <p className="text-xs text-[var(--text-muted)] mb-3">Tick to select, use +/− to adjust quantity</p>
          {availableSizes.length === 0 && effectiveStationId ? (
            <p className="text-sm text-[var(--text-muted)]">Select a station to see available sizes</p>
          ) : (
            <div className="space-y-2.5">
              {availableSizes.map((size) => {
                ...
                const price   = l ? l.fillPrice : null;
        ...
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-3">3. Delivery Schedule</p>
        ...
        {/* Pickup Location */}
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-1">Pickup Location</p>
            ...
            <div className="flex-1 min-w-0">
              {pickupLoc
                ? <><p className="text-sm font-semibold text-[var(--text-primary)] truncate">{pickupLoc.street}</p>
                    <p className="text-xs text-[var(--text-muted)] truncate">{pickupLoc.city}</p></>
        ...
        {/* Delivery Location */}
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-1">Delivery Location</p>
              ...
              <div className="flex-1 min-w-0">
                {deliveryLoc
                  ? <><p className="text-sm font-semibold text-[var(--text-primary)] truncate">{deliveryLoc.street}</p>
                      <p className="text-xs text-[var(--text-muted)] truncate">{deliveryLoc.city}</p></>
        ...
        {/* Step 4 — Photo */}
        <div>
          <p className="text-sm font-bold text-[var(--text-primary)] mb-3">4. Picture of cylinder(s)</p>
          {photo ? (
            <div className="relative">
              ...
              <button onClick={() => { setPhoto(null); if (photoRef.current) photoRef.current.value = ''; }}
                ...
                <Plus className="w-4 h-4 text-[var(--text-primary)] rotate-45" />
              </button>
            </div>
          ) : (
            <button onClick={() => photoRef.current?.click()}
              className="w-full h-44 rounded-2xl border-2 border-dashed border-[var(--border)] flex flex-col items-center justify-center gap-3 transition-all hover:border-brand-500">
      ...
      {/* Bottom summary + CTA */}
      <div className="fixed bottom-0 inset-x-0 bg-[var(--bg-card)] border-t border-[var(--border)] px-4 pt-3 pb-6 z-20 max-w-lg mx-auto">
        <div className="space-y-1 mb-3 text-sm">
          ...
          {totalQty === 0 && (
            <p className="text-[var(--text-muted)] text-xs">No cylinders selected</p>
          )}
        ...
        <button onClick={handleContinue} disabled={totalQty === 0}
          className="w-full h-13 bg-brand-500 hover:bg-brand-600 disabled:opacity-40 text-white rounded-2xl font-bold text-base flex items-center justify-center gap-2 transition-colors py-4">
